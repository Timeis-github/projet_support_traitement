...
# Define other numeric columns to cast (excluding the first 5 and timestamps)
other_cols_to_cast = [c for c in lines_df.columns[5:] if not c.endswith("_TIMESTAMP")]
static_cols = lines_df.columns[:5]

# Build a list of all transformations
transformations = [F.col(c) for c in static_cols] # Keep first 5 as is

# Add cleaned timestamp columns (cast to double AND handle -1)
transformations += [
    F.when(F.col(c).cast("double") > 0, F.col(c).cast("double"))
     .otherwise(None)
     .alias(c) 
    for c in timestamp_cols
]

# Add remaining casted columns
transformations += [F.col(c).cast("double").alias(c) for c in other_cols_to_cast]

# Apply everything in a single select
lines_df = lines_df.select(*transformations)

# 3. Compute row-level min and max using these cleaned expressions
# least/greatest returns null if ALL arguments are null
row_min = F.least(*timestamp_cols)
row_max = F.greatest(*timestamp_cols)

# 4. Add temporary columns and aggregate by jobid
job_execution_times_df = lines_df.withColumn("row_min", row_min) \
                                .withColumn("row_max", row_max) \
                                .groupBy("jobid") \
                                .agg(F.min("row_min").alias("job_start"), 
                                     F.max("row_max").alias("job_end"))

# 5. Calculate final execution time
# coalesce(..., 0) ensures that if (job_end - job_start) is null (all timestamps were invalid), it becomes 0
...
jobids_df = longest_jobids_df.union(largest_jobs_IO_jobids_df).union(largest_users_jobids_df).distinct().sort("jobid")
...
print(jobids_df.count())
print(df_result.count())

missing_jobids = df_result.select("_c0").join(jobids_df, 
                                              df_result._c0 == jobids_df.jobid, 
                                              "left_anti")
missing_jobids.show(50)
print(f"Missing {missing_jobids.count()} jobids")
